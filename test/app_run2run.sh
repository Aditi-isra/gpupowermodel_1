...
declare -a appParams=(" > results/BFS_RUN_OUTPUT" ) # ex

nRuns=$((numRuns-1))
for i in $(seq 0 $nRuns) # 3*1*167 files = 501 output files 
do
    appID=0
    for prog in "${progs[@]}"
    do
	...
	do
	    ...
	    if [[ $s == "Graphics" ]]
	    then
		IFS=' ' read -ra ll <<< ${l[1]}       #reading parameter into ll

		 # GET RESULTS WITH NEW FREQUENCY

		echo "### $prog - ${ll[0]} (MHz) - Iteration: $i ###"

		curPwrLimit=250
		freq=${ll[0]} #reading freq dynamically
		file="results/$prog-$prec-$mode-$curPwrLimit-$arch-$freq-"  #mode - run and load (loadscaling) ,curPwrLimit= max power limit of gpu
		...
		appCmd="$curPwrLimit $freq $base $progPath${progs[$appID]}${appParams[$appID]}" # appCMD is passed as argument to control ,
		#echo $appCmd
		...
		if [[ $prog == "BFS" ]]
		then
		  #  r=$(grep Triad results/BFS_RUN_OUTPUT) # instead of TRIAD write the parameter we need to grep --uncomment 
		  #  echo $r | cut -d" " -f 2 >> results/run_triads_dvfs_$freq # cut floating point to 2 decimal
...
'
	...
	do
	    ...
	    echo "### $prog - $curPwrLimit (W) - Iteration: $i ###"

	    freq=1380 # read max from the supported clocks
	    file="results/$prog-$prec-$mode-$curPwrLimit-$arch-$freq-"
            ...
            appCmd="$curPwrLimit $freq $base $progPath${progs[$appID]}${appParams[$appID]}"
            #echo $appCmd
